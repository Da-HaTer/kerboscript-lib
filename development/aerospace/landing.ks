...
until airspeed<=1 and ish(verticalSpeed,0,0.1){
    if runmode=0{
        ...
        if vdot((pt1- rvec):normalized,pt1:normalized) >=0.1{
            //should make u turn
            ///wrong possibily
            if vdot(rn,rvec) >1 lock pt2 to pt1-rn:normalized*4000+rw_dir.
            else lock pt2 to pt1+rn:normalized*4000+rw_dir.
        }
        ...
        lock rvec1 to rvec-(rvec2-rvec):normalized*1500.
        //margin
        set turnaxis to vcrs(relative_radar_vector(rvec1-pt1),up:vector).
        lock turnvect to (vdot(turnaxis,pt1)*turnaxis):normalized*min(1,vdot(turnaxis,pt1:normalized)).
        ...
        print 3 at (0,5).
        brakes on.
        if max(-10,min(1-1.085^(altitude -170),1))<>0 set descent to relative_radar_vector(3*rvec2-rvec)+tan(max(-10,min(1-1.085^(altitude -170),1)))*up:vector.
...
//issues : over roll and pitch, need to follow vector not accurate enough